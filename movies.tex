...
Table \ref{} shows the list of features along with their frame rate and toolbox used. 
...
All our experiments are performed by using a leave cross-validation scheme.
During cross-validation, we use x movies as the training set, x for validation and 1 movie as the testing set.
The primary metric for the evalauation of our methods is Canonical Correlation Coefficient (CCC) \cite{} between the annotated ground truth and the predicted ratings on the test set.
This metric accounts for both the similarity of pattern between the two time series as well as the differnce of bias between them. 
...
\begin{table}[h]
>>>>>>> 66ece5fbee370c6b66fc599e733698f078889a26
\centering
